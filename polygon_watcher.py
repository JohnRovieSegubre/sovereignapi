...
def watch_loop():
    ...
    # Start mainly for new blocks (ensure we don't scan too far back on restart)
    # We rewind 5 blocks just to be safe in case of immediate restart
    current_block = w3.eth.block_number
    last_processed_block = current_block - 5
    logger.info(f"🚀 Starting Watcher from block {last_processed_block}...")
    ...
    while True:
        try:
            ...
            if current_block > last_processed_block:
                # Safe Mode: Scan small chunk (10 blocks) to respect RPC limits
                # CRITICAL: Stay 15 blocks behind tip to ensure logs are indexed by RPC
                safe_tip = current_block - 15
                
                # If we are ahead of safe tip, just wait.
                if last_processed_block >= safe_tip:
                    time.sleep(POLL_INTERVAL)
                    continue

                to_scan = min(safe_tip, last_processed_block + 10)
                
                logger.info(f"🔍 Scanning blocks {last_processed_block + 1} to {to_scan}...")
                
                # Fetch Logs using direct w3.eth.get_logs for better control
                # filter_params = {
                ...
                # Using contract instance is safer for decoding
