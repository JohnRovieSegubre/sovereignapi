...
def test_full_flow():
    print("🚀 STARTING MAILBOX PROTOCOL TEST")
    print("=================================")
    ...
    print(f"\n📡 [STEP 1] Watcher detects ${amount_usdc} USDC deposit...")
    ...
    if resp.status_code == 200:
        print("✅ Gateway accepted mint request.")
    ...
    try:
        with open(".agent/data/pending_claims.json", "r") as f:
            data = json.load(f)
            if tx_hash in data:
                print("✅ [PERSISTENCE] Token found in pending_claims.json disk storage!")
            else:
                print("❌ [PERSISTENCE] Token NOT found in disk storage!")
                return
    except Exception as e:
        print(f"❌ Could not read pending_claims.json: {e}")
        return

    # 4. Wallet Claims the Token
    print(f"\n👛 [STEP 3] Wallet attempts to claim {tx_hash[:10]}...")
    ...
    if success:
        print("✅ Wallet successfully claimed the token!")
    else:
        print("❌ Wallet failed to claim token.")
        return
    ...
    with open(".agent/data/pending_claims.json", "r") as f:
        ...
        if tx_hash not in data:
            print("✅ [CLEANUP] Token properly removed from mailbox after claim.")
        else:
            print("❌ [CLEANUP] Token still in mailbox! Double-spend risk.")

    # 6. Spend the Token to prove it's valid
    print(f"\n🧠 [STEP 4] Spending token on a thought...")
