...
def run_job(job_payload: dict):
    ...
    try:
        ...
        # SKIPPING SAM for speed as requested.
        start_mask = None 
        end_mask = None
...
if __name__ == "__main__":
    ...
    # Test Payload with Aspect Ratio Mismatch
    # Start: Square (Downloads/start.png is assumed square based on logs)
    # End: Wide (We will point to a different image or just mock the loader if needed, but let's try real files)
    # Actually, easiest way to trigger mismatch with same files is to modify the loader mock or validation logic temporarily? 
    # No, proper way is to use a mismatched file. 
    # Let's create a dummy mismatched file.
    
    dummy_wide = cv2.imread(r"C:\Users\rovie segubre\Downloads\start.png")
    if dummy_wide is not None:
        dummy_wide = cv2.resize(dummy_wide, (1000, 500)) # 2:1 ratio
        cv2.imwrite("test_wide.png", dummy_wide)
        
        test_payload = {
            "start_panel": "C:/Users/rovie segubre/Downloads/start.png",
            "end_panel": "test_wide.png",
            "mode": "final",
            "overrides": {"face": True},
            "timing": {"fps": 24, "frames": 24},
            "seed": 999
        }
        logger.info("Running with ASPECT MISMATCH payload...")
        run_job(test_payload)
    else:
        logger.error("Could not load base image for test.")
