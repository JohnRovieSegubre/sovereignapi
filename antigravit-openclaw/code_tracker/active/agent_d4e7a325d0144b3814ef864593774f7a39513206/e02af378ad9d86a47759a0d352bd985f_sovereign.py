...
class SovereignClient:
            ...
            def create(self, model, messages, max_retries=1):
                ...
                try:
                    ...
                    # --- PAYMENT REQUIRED (402) ---
                        ...
                        if x402_header:
                            print("‚ö° x402 Header Detected. Handling Instant Payment...")
                            signature = self.client._handle_x402_payment(x402_header)
                            if signature:
                                print("üîÅ Retrying request with x402 Signature...")
    ...
    def _handle_x402_payment(self, header_guts):
        ...
        try:
            ...
            # 2. Find a supported scheme (Base Mainnet)
            # In a real agent, we'd check which chains we have funds on.
            # Here we prioritized 'base-mainnet' or 'base-sepolia'.
            target_scheme = None
            for scheme in accepts:
                if "base" in scheme.get("network", ""):
                    target_scheme = scheme
                    break
            
            if not target_scheme:
                print("‚ùå No supported x402 network found (Need Base).")
                return None
            ...
            if not self.private_key:
                print("‚ùå No Private Key for signing.")
                return None
            
            msg = f"I authorize payment of {target_scheme['price']} USDC to {target_scheme.get('payTo')} on {target_scheme['network']}"
            signed_msg = self.account.sign_message(json.dumps(msg).encode("utf-8"))
...
‰ *cascade08‰Á*cascade08ÁÓ *cascade08ÓÔ*cascade08Ô *cascade08ı*cascade08ıˆ *cascade08ˆ¸*cascade08¸˝ *cascade08˝ô*cascade08ôö *cascade08öú*cascade08úù *cascade08ù†*cascade08†¢ *cascade08¢§*cascade08§¶ *cascade08¶Ω*cascade08Ωï *cascade08ï…q*cascade08…qõs *cascade08"(d4e7a325d0144b3814ef864593774f7a395132062Efile:///c:/Users/rovie%20segubre/agent/sovereign_api/sdk/sovereign.py:&file:///c:/Users/rovie%20segubre/agent