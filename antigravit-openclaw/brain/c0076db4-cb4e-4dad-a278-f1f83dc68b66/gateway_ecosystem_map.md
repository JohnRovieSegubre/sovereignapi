...
-   **`agent/scripts/verify_alby.py`**  
     Checks if the Alby token is valid.
-   **`agent/scripts/test_invoice_generation.py`**  
...
-   **`agent/moltbook_explore.py`** (Reads social posts)
-   **`agent/post_moltbook.py`** (Posts content)
-   **`agent/post_and_verify_moltbook.py`** (Posts + Math Challenge)
-   **`agent/check_moltbook_status.py`** (Checks agent status)
-   **`agent/create_submolt.py`** (Creates communities)
-   **`agent/list_moltbook_submolts.py`** (Lists communities)
-   **`agent/debug_moltbook_raw.py`** (Raw API debugging)
