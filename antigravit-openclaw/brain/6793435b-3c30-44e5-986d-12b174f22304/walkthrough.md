...
|:-----|:-----------------|
| `sdk/sovereign.py` | Hardcoded IP `34.55.175.24`, wallet `0xC8Dc...` |
| `sdk/agent_customer.py` | Hardcoded IP, wallet |
| `polygon_watcher.py` | Hardcoded wallet, Alchemy RPC URL |
| `gateway_server.py` | `default_unsafe_secret_for_dev` |
| `cloud_mint.py` | Hardcoded IP `34.55.175.24`, fake admin key |
| `deploy_to_cloud.ps1` | Hardcoded IP, username |

### New Files Created (3)

| File | Purpose |
|:-----|:--------|
| `.gitignore` | Protects secrets, wallet files, `__pycache__` |
...
```
> Paste from local `sovereign_api/gateway_server.py` → Save (`Ctrl+X, Y, Enter`)

```bash
rm polygon_watcher.py && nano polygon_watcher.py
```
> Paste from local `sovereign_api/polygon_watcher.py` → Save

```bash
rm -rf sdk && mkdir sdk && nano sdk/__init__.py
```
> Paste `sdk/__init__.py` content → Save

```bash
nano sdk/sovereign.py
```
> Paste from local `sovereign_api/sdk/sovereign.py` → Save
