...
def run_monitor():
    ...
    def check_website():
        ...
        try:
            ...
            logging.info(f"🌐 Website Health: {resp.status_code} OK | {len(resp.content)} bytes")
            if resp.status_code != 200:
                logging.error(f"⚠️ Website returned {resp.status_code}: {resp.text[:100]}")
        except Exception as e:
            logging.error(f"❌ Website Poll Failed: {e}")
